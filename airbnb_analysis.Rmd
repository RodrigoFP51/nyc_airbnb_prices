...
```
- Manhattan demonstra ter preços bem maiores, com seu segundo quintil sendo maior que o quinto quintil de Bronx.
...
airbnb %>% 
  ...
  group_by(year,month) %>% 
  summarize(price = median(price), .groups = "drop") %>% 
  ggplot(aes(year, month, fill = price)) +
  geom_tile(linejoin = "round", color = "white") +
  geom_text(aes(label = scales::dollar(price)), fontface = "bold", color = "white") +
  scale_x_continuous(breaks = 2011:2019) +
  scale_fill_paletteer_c("scico::bamako", name = "Price") +
  theme_minimal() +
  labs(x='', y='',
...
```{r}
airbnb %>% 
  transmute(number_of_reviews, minimum_nights, reviews_per_month) 
  #as.data.frame()
```


- Impute missing last_review e reviews_per_month
