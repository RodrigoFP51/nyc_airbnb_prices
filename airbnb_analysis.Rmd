...
list(k = c(2,4,6)) %>%
  map(
    \(x){
      airbnb %>% 
        as.data.frame() %>% 
        simputation::impute_knn(reviews_per_month ~ minimum_nights + number_of_reviews, k = x) %>% 
        as_tibble() %>% 
        fill(last_review, .direction = "down") %>% 
        drop_na() %>% 
...
## Modelagem tidymodels

```{r}
library(tidymodels)
library(textrecipes)

df <- read_csv(here("Data/airbnb_imputed.csv")) %>% 
  mutate(across(where(is.character), factor)) %>% 
  select(-id, -host_id, -host_name) %>% 
  filter(price > 0)

split <- initial_split(df, prop = 7/10, strata = price)
df_train <- training(split)
df_test <- testing(split)

df_folds <- vfold_cv(df_train, v = 5, strata = price)
```

```{r}
base_rec <- recipe(price ~ ., data = df_train) %>% 
  step_log(price) %>% 
  step_other(neighbourhood, other = "Other", threshold = 0.03) %>% 
  step_date(last_review, 
            features = c("month", "dow", "week"),
            label = TRUE,
...
rec_2 <- base_rec %>% 
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_tokenize(name) %>% 
  step_stopwords(name, stopword_source = "snowball") %>% 
  step_tokenfilter(name, min_times = 1, max_tokens = 50) %>%
  step_tfidf(name) %>% 
  step_dummy(all_nominal_predictors(), one_hot = TRUE) %>% 
  step_zv(all_predictors())
